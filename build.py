...
ACQ={"TR","SFA","CLW","R5"}; AMA={"SGN","DR"}; CUT={"REL","OUT","DES","NT"}
    ...
    for rnd in d["drafts"]["rounds"]:
        for p in rnd.get("picks", []):
            ...
            if pid not in drafted or y > drafted[pid][0]:
                ...
                draftrd[pid] = str(p.get("pickRound", "") or "")
            if y >= 2015:
...
for t in tx:
    ...
    if code in ("TR", "CLW", "R5"):
        ft = FRMAP.get((t.get("fromTeam") or {}).get("name", ""))
        if ft: fromteam[(pid, date, team)] = ft
    names[pid] = per.get("fullName", "")
# ---- frozen acquisition history (2009-2014 API + pre-2009 Retrosheet), one-time sidecar ----
import os as _os
_acqp = next((p for p in ("acq_history.json", "acq_history.json.json") if _os.path.exists(p)), None)
FUNRES = 0
if _acqp:
    _ah = json.load(open(_acqp))
    _ns = 0
    _norm = {"ATH": "OAK"}
    for _pid, _dt, _code, _tm2, _ft2, _mi in _ah.get("events", []):
        if _code not in ACQ and _code not in AMA: continue
        _tm2 = _norm.get(_tm2, _tm2); _ft2 = _norm.get(_ft2, _ft2)
        if _tm2 not in TEAMS: continue
        events[_pid].append((_dt, "AMA" if _code in AMA else _code, _tm2, bool(_mi)))
        if _code in ("TR", "CLW", "R5") and _ft2: fromteam[(_pid, _dt, _tm2)] = _ft2
        _ns += 1
    for _pid, _dt in _ah.get("cuts", []): cuts[_pid].append(_dt)
    FUNRES = len(_ah.get("unresolved", []))
    print("sidecar:", _ns, "events merged |", FUNRES, "unresolved pre-2009 stints", flush=True)
else:
    print("sidecar MISSING: acq_history.json — pre-2015 acquisitions incomplete", flush=True)

_drop = []
for pid in events:
    events[pid] = list(dict.fromkeys(sorted(events[pid])))
    _out = []
    _base = drafted[pid][1] if pid in drafted else None
    for _e in events[pid]:
        if _e[1] == "SFA":
            _prev = _out[-1][2] if _out else _base
            if _prev == _e[2]:
                continue  # re-signed with the club he never left: original acquisition keeps the credit
        _out.append(_e)
    if _out: events[pid] = _out
    else: _drop.append(pid)
for pid in _drop: del events[pid]
...
for blob, pit in ((war_bat, False), (war_pitch, True)):
    for r in csv.DictReader(io.StringIO(blob)):
        ...
        tid = r.get("team_ID", ""); fr = {"FLA":"MIA","TBD":"TBR","ANA":"LAA","MON":"WSN","ATH":"OAK"}.get(tid, tid)
...
# reclassify Rule 5 selections the feed mislabels as December waiver claims of never-debuted players
_r5n = 0
for _pid, _evs in events.items():
    for _ix, _e in enumerate(_evs):
        if _e[1] == "CLW" and "12-04" <= _e[0][5:] <= "12-16" and not prior_mlb(_pid, _e[0]):
            _evs[_ix] = (_e[0], "R5", _e[2], _e[3]); _r5n += 1
print("rule5 reclassified from waiver:", _r5n, flush=True)
...
detail = {t: dict(draft=[], intl=[], tr_pre=[], tr_mlb=[], fa=[], scrap=[], waiver=[], r5=[], away=[]) for t in TEAMS}
...
def bucket(pe, era):
    for (pid, date, kind, team, minor), w in pe.items():
        ...
        if k == "R5":
            A[team]["r5"] += wp
            if era:
                tops[(team, "5")].append((wp, nm))
                if wp >= 0.5: detail[team]["r5"].append(dict(n=nm, y=yr, f=fromteam.get((pid, date, team), ""), w=S(wp), s=sv))
            continue
        if k == "TR":
...
for t in TEAMS:
    ...
    acq_out.append(dict(team=t, trade=S(a["trade"]), trade26=S(b["trade"]), trade_milb=S(a["trade_milb"]),
        trade_mlb=S(a["trade_mlb"]), fa=S(a["fa"]), fa26=S(b["fa"]), fa_scrap=S(a["fa_scrap"]), r5=S(a["r5"]),
...
def classify(pid, fr):
    ...
    if cand:
        ...
        if k == "R5": return "Rule 5 pick", date[:4]
        if k == "SFA":
...
draftold = {t: [] for t in TEAMS}
for pid, (y, t, pk, nm) in drafted.items():
    if 2000 <= y <= 2014:
        wp = max(0.0, cwALL.get(pid, 0.0))
        if wp >= 2:
            draftold[t].append([names.get(pid, nm) or nm, y, pk, S(wp), S(w26.get(pid, 0.0))])
for t in TEAMS: draftold[t] = sorted(draftold[t], key=lambda r: -r[3])[:40]
...
for y, d in draft_json.items():
    for rnd in d["drafts"]["rounds"]:
        for p in rnd.get("picks", []):
            ...
            wp = max(0.0, cwALL.get(pid, cw.get(pid, 0.0)))
            if wp < 0.5: continue
            nmm = names.get(pid, "") or per.get("fullName", "")
            typ = ""
            try:
            ...
            sn = ((p.get("school") or {}).get("name", "") or "").strip()
            snl = sn.lower()
            if sn and ("community college" in snl or "junior college" in snl or snl.endswith(" cc") or snl.endswith(" jc")):
                typ = "JC"
            if sn:
                schools.setdefault(sn, []).append([nmm, y, p.get("pickNumber", 0) or 0,
                                                   drafted[pid][1], S(wp), S(w26.get(pid, 0.0)), typ])
            st = per.get("birthStateProvince", "")
            if st and per.get("birthCountry") in ("USA", "Puerto Rico"):
                geo[st].append([nmm, y, sn, typ, S(wp), S(w26.get(pid, 0.0)), drafted[pid][1]])
for st in geo: geo[st] = sorted(geo[st], key=lambda r: -r[4])[:250]
for sn in schools: schools[sn].sort(key=lambda r: (-r[4], r[1]))

# ---------------- farm (seasons-windowed 2015-2026) ----------------
TEAMSET = set(TEAMS)
_oocache = {}
def _oo(pid):
    if pid not in _oocache: _oocache[pid] = original_org(pid)
    return _oocache[pid]
_cells = {}   # (pid, fr, y) -> (chan, evdate, ft)
chy = collections.defaultdict(float); losty = collections.defaultdict(float); salvy = collections.defaultdict(float)
for (m, fr, y), w in rows.items():
    evs = events.get(m); cutoff = f"{y}-10-05"; cand = None
    if evs:
        for e in reversed(evs):
            if e[0] <= cutoff:
                if e[2] == fr: cand = e
    ...
    if cand:
        ...
        elif k == "AMA": ch = "H" if _oo(m) == fr else "F"
        else: ch = "F"
        _cells[(m, fr, y)] = (ch, cand[0], fromteam.get((m, cand[0], fr), ""), bool(cand[3]) if len(cand) > 3 else False)
        chy[(fr, ch, y)] += w
        if ch == "T":
            _ft3 = fromteam.get((m, cand[0], fr), "")
            if _ft3: losty[(_ft3, y)] += w
        elif ch in ("W", "R"): salvy[(fr, y)] += w
        elif ch == "F":
            try: _scr = (bool(cand[3]) if len(cand) > 3 else False) or recent_cut(m, cand[0])
            except Exception: _scr = False
            if _scr: salvy[(fr, y)] += w
    else:
        _ch0 = "H" if _oo(m) == fr else "U"
        _cells[(m, fr, y)] = (_ch0, "", "", False)
        chy[(fr, _ch0, y)] += w
# origin meta for H rows and capital display
_hpids = {m for (m, fr, y), (ch, *_ ) in _cells.items() if ch in ("H",)}
_nond = [m for m in _hpids if m not in drafted]
_ctry = {}
for _i in range(0, len(_nond), 100):
    _ids = ",".join(map(str, _nond[_i:_i + 100]))
    try:
        for _per in jget("https://statsapi.mlb.com/api/v1/people?personIds=" + _ids).get("people", []):
            _ctry[_per["id"]] = _per.get("birthCountry", "")
    except Exception: pass
def _ometa(m):
    if m in drafted:
        _y2, _t2, _pk, _nm2 = drafted[m]; _rd = draftrd.get(m, "")
        return f"{_y2} draft" + (f" \u00b7 rd {_rd}" if _rd else "") + (f" \u00b7 pk {_pk}" if _pk != 9999 else "")
    _bc = _ctry.get(m, "")
    return "UDFA signing" if _bc in ("USA", "Puerto Rico") else ("Intl signing" if _bc else "Intl/UDFA signing")
# stint rows: (pid, fr, ch, evdate) -> arrays 2015..CUR
_st = {}; _stmeta = {}
for (m, fr, y), w in rows.items():
    ch, ed, ft, mi = _cells[(m, fr, y)]
    k = (m, fr, ch, ed)
    if k not in _st: _st[k] = [0] * (CUR - 2015 + 1); _stmeta[k] = (ft, mi)
    _st[k][y - 2015] += int(round(w * 10))
fsp = []
for (m, fr, ch, ed), arr in _st.items():
    ft, mi = _stmeta.get((m, fr, ch, ed), ("", False))
    pres = 0
    for yy, ff in seasons.get(m, ()):
        if ff == fr and 2015 <= yy <= CUR: pres |= 1 << (yy - 2015)
    o = _oo(m) or ""
    if ch == "H": meta = _ometa(m); og = ""
    elif ch == "T":
        yy = ed[:4] if ed else "?"
        meta = f"trade {yy}" + (f" \u00b7 from {ft}" if ft else "")
        og = o if (ft and o == ft) else ""
    elif ch == "F":
        scrap = False
        if ed:
            try: scrap = mi or recent_cut(m, ed)
            except Exception: scrap = mi
        meta = ("reclamation " if scrap else "FA ") + (ed[:4] if ed else "")
        og = ""
    elif ch == "W": meta = f"waiver {ed[:4] if ed else '?'}" + (f" \u00b7 from {ft}" if ft else ""); og = ""
    elif ch == "R": meta = f"Rule 5 {ed[:4] if ed else '?'}" + (f" \u00b7 from {ft}" if ft else ""); og = ""
    else: meta = "acquired pre-2009 \u00b7 unresolved"; og = ""
    fsp.append([names.get(m, str(m)), fr, ch, og, meta, pres, arr])
print("fsp stint rows:", len(fsp), flush=True)

# ---------------- OTR: Organizational Talent Rating ----------------
mm = lambda lst, k: {r["team"]: r[k] for r in lst}
surM = mm(surplus, "surplus"); netM = mm(net, "net"); intlM = mm(intl_out, "war")
scrapM = mm(acq_out, "fa_scrap"); wavM = {r["team"]: r["waiver"] + r["r5"] for r in acq_out}
D_full = {t: surM[t] + intlM[t] for t in TEAMS}
T_full = dict(netM)
S_full = {t: scrapM[t] + wavM[t] for t in TEAMS}
RY = set(range(CUR - 2, CUR + 1))
D_rec = {t: sum(chy.get((t, "H", y), 0.0) for y in RY) for t in TEAMS}
T_rec = {t: sum(chy.get((t, "T", y), 0.0) for y in RY) - sum(losty.get((t, y), 0.0) for y in RY) for t in TEAMS}
S_rec = {t: sum(salvy.get((t, y), 0.0) for y in RY) for t in TEAMS}
import statistics as _stx
def _zidx(d):
    vs = list(d.values()); mu = _stx.fmean(vs); sd = _stx.pstdev(vs) or 1.0
    return {t: 100 + 15 * (d[t] - mu) / sd for t in d}
_pools = {"D": sum(max(0, v) for v in D_full.values()), "T": sum(max(0, v) for v in T_full.values()), "S": sum(max(0, v) for v in S_full.values())}
_ps = sum(_pools.values()) or 1.0
wD, wT, wS = _pools["D"] / _ps, _pools["T"] / _ps, _pools["S"] / _ps
Df, Tf, Sf = _zidx(D_full), _zidx(T_full), _zidx(S_full)
Dr, Tr, Sr = _zidx(D_rec), _zidx(T_rec), _zidx(S_rec)
otr_full = {t: wD * Df[t] + wT * Tf[t] + wS * Sf[t] for t in TEAMS}
otr_rec = {t: wD * Dr[t] + wT * Tr[t] + wS * Sr[t] for t in TEAMS}
_sf = 15 / (_stx.pstdev(list(otr_full.values())) or 15)
_sr = 15 / (_stx.pstdev(list(otr_rec.values())) or 15)
otr_full = {t: 100 + (otr_full[t] - 100) * _sf for t in TEAMS}
otr_rec = {t: 100 + (otr_rec[t] - 100) * _sr for t in TEAMS}
TREND_TH = 8
grades = []
for t in sorted(TEAMS, key=lambda x: -(0.7 * otr_full[x] + 0.3 * otr_rec[x])):
    dv = otr_rec[t] - otr_full[t]
    grades.append(dict(team=t, grade=round(0.7 * otr_full[t] + 0.3 * otr_rec[t]),
        d=round(Df[t]), t2=round(Tf[t]), s=round(Sf[t]), rec=round(otr_rec[t]),
        dr=round(Dr[t]), tr=round(Tr[t]), sr=round(Sr[t]),
        comp=round(otr_full[t]),
        trend=(1 if dv >= TREND_TH else (-1 if dv <= -TREND_TH else 0))))
print("OTR weights D/T/S:", round(wD, 2), round(wT, 2), round(wS, 2), "| trend \u00b1", TREND_TH, flush=True)

# ---------------- class grades (Class+) + top 15 ----------------
_cyv = []
for t in TEAMS:
    for ys, plist in classes.get(t, {}).items():
        y = int(ys)
        if 2015 <= y <= 2023:
            _cyv.append([t, y, sum(max(0.0, p[3]) for p in plist)])
_ymean = {}
for y in range(2015, 2024):
    vs = [r[2] for r in _cyv if r[1] == y]
    _ymean[y] = (sum(vs) / len(vs)) if vs else 1.0
clsplus = {}; _tops = []
for t, y, wv in _cyv:
    plus = round(100 * wv / (_ymean[y] or 1.0))
    clsplus[f"{t}|{y}"] = plus
    hh = [p[2] for p in sorted(classes[t][str(y)], key=lambda p: -p[3])[:3] if p[3] >= 1]
    _tops.append([t, y, plus, S(wv), hh])
_tops.sort(key=lambda r: -r[2])
top15 = _tops[:15]
print("class grades:", len(clsplus), "| #1:", top15[0][:4] if top15 else None, flush=True)
...
data = dict(asof=TODAY.strftime("%B %d, %Y"), games=GAMES, grades=grades, draft=draftT, recent=recent,
    ...
    abdet=abdet, hits=hits, geo=dict(geo), origros=origros, schools=schools, draftold=draftold, fsp=fsp, funres=FUNRES, fspy0=2015, clsplus=clsplus, top15=top15)
